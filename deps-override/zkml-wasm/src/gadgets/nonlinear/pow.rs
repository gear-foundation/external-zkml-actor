...
impl<F: PrimeField> NonLinearGadget<F> for PowGadgetChip<F> {
  fn generate_map(scale_factor: u64, min_val: i64, num_rows: i64) -> HashMap<i64, i64> {
    ...
    for i in 0..num_rows {
      ...
      let y: i64 = (shifted * shifted * shifted) / ((scale_factor * scale_factor) as i64);
  ...
  fn get_selector(&self) -> halo2_proofs_wasm::plonk::Selector {
...
impl<F: PrimeField> Gadget<F> for PowGadgetChip<F> {
  ...
  fn forward(
    &self,
    layouter: impl halo2_proofs_wasm::circuit::Layouter<F>,
    vec_inputs: &Vec<Vec<&AssignedCell<F, F>>>,
    single_inputs: &Vec<&AssignedCell<F, F>>,
  ) -> Result<Vec<AssignedCell<F, F>>, Error> {
