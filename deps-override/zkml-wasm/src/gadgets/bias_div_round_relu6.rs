use gstd::prelude::*;
use gstd::{collections::BTreeMap, marker::PhantomData, rc::Rc};
use gstd::{ToOwned, ToString};

use halo2_proofs_wasm::{
  circuit::{AssignedCell, Layouter, Region, Value},
  halo2curves_wasm::ff::PrimeField,
...
pub static mut VKEY_MAP: Option<HashMap<i64, i64>> = None;
//pub static mut VKEY_MAP_LAST_NUM: i64 = 0;

// Call ~52 times to fill map
// pub fn fill_vkey_map() {
//   unsafe {
//     if VKEY_MAP.is_none() {
//       VKEY_MAP = Some(
//         (0..32759)
//           .map(|i| (i, (i - 16384).clamp(0, 6 * 512 as i64)))
//           .collect(),
//       );
//       VKEY_MAP_LAST_NUM = 32759;
//       return;
//     }

//     let inc = match VKEY_MAP_LAST_NUM {
//       _ if VKEY_MAP_LAST_NUM < 15000 => 1000,
//       _ if VKEY_MAP_LAST_NUM < 29000 => 500,
//       _ => 250,
//     };

//     let upper_limit = (VKEY_MAP_LAST_NUM + inc).min(32759);

//     VKEY_MAP
//       .as_mut()
//       .unwrap()
//       .extend((VKEY_MAP_LAST_NUM..upper_limit).map(|i| (i, (i - 16384).clamp(0, 6 * 512 as i64))));

//     VKEY_MAP_LAST_NUM += inc;
//   }
// }

pub fn get_vkey_map() -> HashMap<i64, i64> {
  (0..32759)
    .map(|i| (i, (i - 16384).clamp(0, 6 * 512 as i64)))
    .collect()
}
...
impl<F: PrimeField> BiasDivRoundRelu6Chip<F> {
  ...
  pub fn get_map(scale_factor: u64, min_val: i64, num_rows: i64) -> HashMap<i64, i64> {
    assert_eq!(scale_factor, 512);
    assert_eq!(min_val, -16384);
    assert_eq!(num_rows, 32759);

    // let map = (0..(num_rows / 4))
    //   .map(|i| (i, (i + min_val).clamp(0, 6 * scale_factor as i64)))
    //   .collect();
