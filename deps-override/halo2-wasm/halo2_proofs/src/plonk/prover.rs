...
use gstd::ops::RangeTo;
use gstd::{iter, mem};
use halo2curves_wasm::CurveExt;
use no_std_compat::sync::atomic::AtomicUsize;
    ...
    Scheme::Scalar: WithSmallOrderMulGroup<3> + FromUniformBytes<64>,
{
    for instance in instances.iter() {
        if instance.len() != pk.vk.cs.num_instance_columns as usize {
    ...
    struct WitnessCollection<'a, F: Field> {
        ...
        _marker: gstd::marker::PhantomData<F>,
    }
    ...
    let (advice, challenges) = {
        let mut advice = vec![
            AdviceSingle::<Scheme::Curve, LagrangeCoeff> {
                advice_polys: vec![domain.empty_lagrange(); meta.num_advice_columns as usize],
                advice_blinds: vec![Blind::default(); meta.num_advice_columns as usize],
            };
            instances.len()
        ];
        let mut challenges =
            HashMap::<usize, Scheme::Scalar>::with_capacity(meta.num_challenges as usize);
                ...
                circuits.iter().zip(advice.iter_mut()).zip(instances)
            {
                let mut witness = WitnessCollection {
                    ...
                    advice: vec![
                        ...
                        meta.num_advice_columns as usize
                    ],
                    instances,
        ...
        assert_eq!(challenges.len(), meta.num_challenges as usize);
        let challenges = (0..meta.num_challenges)
            .map(|index| challenges.remove(&(index as usize)).unwrap())
            .collect::<Vec<_>>();
