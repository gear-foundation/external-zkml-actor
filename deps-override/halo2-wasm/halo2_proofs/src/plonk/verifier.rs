...
use crate::halo2curves_wasm::bn256::{Bn256, G1Affine};
use crate::poly::commitment::{CommitmentScheme, Verifier};
use crate::poly::kzg::msm::MSMKZG;
use crate::poly::kzg::msm::{DualMSM, DualMSMData};
use crate::poly::kzg::multiopen::shplonk::{IntermediateSets, IntermediateSetsData};
use crate::poly::kzg::multiopen::{shplonk::construct_intermediate_sets, shplonk::VerifierSHPLONK};
...
enum CommitmentDereference<C: CurveAffine, M: MSM<C>> {
    Commitment(C),
    MSM(M),
}

/// Returns a boolean indicating whether or not the proof is valid
pub fn verify_proof_stage_1<
    'params,
    E: EncodedChallenge<G1Affine>,
    T: TranscriptRead<G1Affine, E>,
>(
    params: &'params <KZGCommitmentScheme<Bn256> as CommitmentScheme>::ParamsVerifier,
    vk: &VerifyingKey<G1Affine>,
    instances: &[&[&[<KZGCommitmentScheme<Bn256> as CommitmentScheme>::Scalar]]],
    transcript: &mut T,
) -> Result<
    IntermediateSetsData<
        <Bn256 as Engine>::Scalar,
        //VerifierQuery<'params, <Bn256 as Engine>::G1Affine, MSMKZG<Bn256>>,
    >,
    Error,
> {
    // Check that instances matches the expected number of instance columns
    for instances in instances.iter() {
        if instances.len() != vk.cs.num_instance_columns as usize {
            return Err(Error::InvalidInstances);
        }
    }

    let instance_commitments = if false {
        instances
            .iter()
            .map(|instance| {
                instance
                    ...
                    .map(|instance| {
                        ...
                        poly.resize(
                            params.n() as usize,
                            <KZGCommitmentScheme<Bn256> as CommitmentScheme>::Scalar::ZERO,
                        );
    ...
    let (advice_commitments, challenges) = {
        let mut advice_commitments =
            vec![vec![G1Affine::default(); vk.cs.num_advice_columns as usize]; num_proofs as usize];
        let mut challenges = vec![
            <KZGCommitmentScheme<Bn256> as CommitmentScheme>::Scalar::ZERO;
            vk.cs.num_challenges as usize
        ];
    ...
    let instance_evals = if false {
        let res = (0..num_proofs)
            .map(|_| -> Result<Vec<_>, _> {
    ...
    let vanishing = {
        ...
        let l_blind: <KZGCommitmentScheme<Bn256> as CommitmentScheme>::Scalar =
            l_evals[1..(1 + blinding_factors)].iter().fold(
                <KZGCommitmentScheme<Bn256> as CommitmentScheme>::Scalar::ZERO,
                |acc, eval| acc + eval,
            );
    ...
    let data = construct_intermediate_sets(queries).into_data();
