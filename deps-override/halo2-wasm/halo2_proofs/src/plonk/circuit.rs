...
use ff::Field;
use gstd::cmp::Ordering;
use gstd::fmt::{Debug, Formatter};
use gstd::{
    convert::TryFrom,
    ops::{Neg, Sub},
};
...
pub trait ColumnType:
    'static + Sized + Copy + gstd::fmt::Debug + PartialEq + Eq + Into<Any>
{
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, codec::Decode, codec::Encode)]
...
impl<C: ColumnType> Column<C> {
    ...
    pub fn query_cell<F: Field>(&self, at: Rotation) -> Expression<F> {
        self.column_type.query_cell(self.index as usize, at)
    }
...
impl<C: ColumnType> Ord for Column<C> {
    fn cmp(&self, other: &Self) -> gstd::cmp::Ordering {
        ...
        match self.column_type.into().cmp(&other.column_type.into()) {
            ...
            gstd::cmp::Ordering::Equal => self.index.cmp(&other.index),
            order => order,
        }
    }
}
...
impl<C: ColumnType> PartialOrd for Column<C> {
    fn partial_cmp(&self, other: &Self) -> Option<gstd::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
...
pub(crate) mod sealed {
    ...
    #[derive(
        Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, codec::Decode, codec::Encode,
    )]
...
/// An advice column
#[derive(Clone, Copy, Eq, PartialEq, Hash, codec::Decode, codec::Encode)]
pub struct Advice {
...
impl gstd::fmt::Debug for Advice {
    fn fmt(&self, f: &mut gstd::fmt::Formatter<'_>) -> gstd::fmt::Result {
...
impl Ord for Any {
    fn cmp(&self, other: &Self) -> gstd::cmp::Ordering {
        ...
        // orderings.
        match (self, other) {
            (Any::Instance, Any::Instance) | (Any::Fixed, Any::Fixed) => gstd::cmp::Ordering::Equal,
            ...
            | (Any::Instance, Any::Fixed) => gstd::cmp::Ordering::Less,
            (Any::Fixed, Any::Instance)
            | (Any::Fixed, Any::Advice(_))
            | (Any::Advice(_), Any::Instance) => gstd::cmp::Ordering::Greater,
        }
    }
}
...
/// ```
/// use halo2_proofs_wasm::poly::Rotation;
/// # use halo2curves_wasm::pasta::Fp;
/// # use halo2_proofs_wasm::plonk::ConstraintSystem;
///
...
/// };
/// use ff::Field;
/// # use halo2_proofs_wasm::plonk::Fixed;
///
/// struct Config {
///     a: Column<Advice>,
///     b: Column<Advice>,
...
/// }
/// ```
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, codec::Encode, codec::Decode)]
...
impl<F: Field> Expression<F> {
        ...
        match self {
            ...
            Expression::Fixed(query) => {
                if query.index.is_none() {
                    let col = Column {
                        index: query.column_index as u64,
                        column_type: Fixed,
                    };
    ...
    fn write_identifier(&self, writer: &mut String) -> io::Result<()> {
        match self {
            Expression::Constant(scalar) => *writer += &format!("{:?}", scalar),
            Expression::Selector(selector) => *writer += &format!("selector[{}]", selector.0),
            Expression::Fixed(query) => {
                *writer += &format!("fixed[{}][{}]", query.column_index, query.rotation.0)
            }
            Expression::Advice(query) => {
                *writer += &format!("advice[{}][{}]", query.column_index, query.rotation.0)
            }
            Expression::Instance(query) => {
                *writer += &format!("instance[{}][{}]", query.column_index, query.rotation.0)
            }
            Expression::Challenge(challenge) => {
                *writer += &format!("challenge[{}]", challenge.index())
            }
            Expression::Negated(a) => {
                *writer += "(-";
...
impl<F: gstd::fmt::Debug> gstd::fmt::Debug for Expression<F> {
    fn fmt(&self, f: &mut gstd::fmt::Formatter<'_>) -> gstd::fmt::Result {
        match self {
...
///
/// ```
/// use halo2_proofs_wasm::{plonk::{Constraints, Expression}, poly::Rotation};
/// use halo2curves_wasm::pasta::Fp;
/// # use halo2_proofs_wasm::plonk::ConstraintSystem;
///
/// # let mut meta = ConstraintSystem::<Fp>::default();
...
/// ```
///
/// Note that the use of `gstd::array::IntoIter::new` is only necessary if you need to
...
type ConstraintsIterator<F, C, I> = gstd::iter::Map<
    gstd::iter::Zip<gstd::iter::Repeat<Expression<F>>, I>,
    ...
    for Constraints<F, C, Iter>
{
    ...
    fn into_iter(self) -> Self::IntoIter {
        gstd::iter::repeat(self.selector)
...
impl<'a, F: Field> gstd::fmt::Debug for PinnedConstraintSystem<'a, F> {
    fn fmt(&self, f: &mut gstd::fmt::Formatter<'_>) -> gstd::fmt::Result {
...
impl<'a, F: Field> gstd::fmt::Debug for PinnedGates<'a, F> {
    fn fmt(&self, f: &mut gstd::fmt::Formatter<'_>) -> Result<(), gstd::fmt::Error> {
        f.debug_list()
...
impl<F: Field> ConstraintSystem<F> {
    ...
    pub(crate) fn query_advice_index(&mut self, column: Column<Advice>, at: Rotation) -> usize {
        ...
        self.num_advice_queries[column.index as usize] += 1;
    ...
    pub fn compress_selectors(mut self, selectors: Vec<Vec<bool>>) -> (Self, Vec<Vec<F>>) {
        ...
        assert_eq!(selectors.len(), self.num_selectors as usize);
        ...
        for expr in self.gates.iter().flat_map(|gate| gate.polys.iter()) {
            if let Some(selector) = expr.extract_simple_selector() {
                degrees[selector.0 as usize] = max(degrees[selector.0 as usize], expr.degree());
            }
        }
        ...
        let (polys, selector_assignment) = compress_selectors::process(
            ...
            || {
                ...
                Expression::Fixed(FixedQuery {
                    ...
                    column_index: column.index as usize,
        ...
        fn replace_selectors<F: Field>(
            ...
            *expr = expr.evaluate(
                ...
                &|selector| {
                    ...
                    selector_replacements[selector.0 as usize].clone()
                },
        ...
        AR: Into<String>,
    {
        ...
        self.general_column_annotations.insert(
            metadata::Column::from((Any::Fixed, column.inner().index as usize)),
            annotation().into(),
        );
    }
        ...
        T: Into<Column<Any>>,
    {
        ...
        // We don't care if the table has already an annotation. If it's the case we keep the new one.
        self.general_column_annotations.insert(
            metadata::Column::from((col_any.column_type, col_any.index as usize)),
            annotation().into(),
        );
    }
    ...
    pub fn fixed_column(&mut self) -> Column<Fixed> {
        let tmp = Column {
            index: self.num_fixed_columns as u64,
            column_type: Fixed,
        };
    ...
    pub fn advice_column_in<P: Phase>(&mut self, phase: P) -> Column<Advice> {
        ...
        let tmp = Column {
            index: self.num_advice_columns as u64,
    ...
    pub fn instance_column(&mut self) -> Column<Instance> {
        let tmp = Column {
            index: self.num_instance_columns as u64,
            column_type: Instance,
        };
    ...
    pub fn challenge_usable_after<P: Phase>(&mut self, phase: P) -> Challenge {
        ...
        let tmp = Challenge {
            index: self.num_challenges as usize,
            phase,
        };
    ...
    pub fn degree(&self) -> usize {
        ...
        gstd::cmp::max(degree, self.minimum_degree.unwrap_or(1) as usize)
    }
    ...
    pub fn blinding_factors(&self) -> usize {
        ...
        let factors = gstd::cmp::max(3, factors);
    ...
    pub fn num_instance_columns(&self) -> usize {
        self.num_instance_columns as usize
    }

    /// Returns number of selectors
    pub fn num_selectors(&self) -> usize {
        self.num_selectors as usize
    }

    /// Returns number of challenges
    pub fn num_challenges(&self) -> usize {
        self.num_challenges as usize
    }
