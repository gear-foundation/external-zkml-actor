...
use gstd::convert::TryInto;
use sha3::{Digest, Keccak256};

use halo2curves_wasm::{Coordinates, CurveAffine};
    ...
    C::Scalar: FromUniformBytes<64>,
{
    fn read_point(&mut self) -> io::Result<C> {
        ...
        let point: C =
            Option::from(C::from_bytes(&compressed)).expect("invalid point encoding in proof");
    ...
    fn read_scalar(&mut self) -> io::Result<C::Scalar> {
        ...
        let scalar: C::Scalar = Option::from(C::Scalar::from_repr(data))
            .expect("invalid field element encoding in proof 1");
    ...
    C::Scalar: FromUniformBytes<64>,
{
    ...
    fn read_scalar(&mut self) -> io::Result<C::Scalar> {
        ...
        self.reader.read_exact(data.as_mut())?;
        let scalar: C::Scalar = Option::from(C::Scalar::from_repr(data))
            .expect("invalid field element encoding in proof 2");
        self.common_scalar(scalar)?;
    ...
    C::Scalar: FromUniformBytes<64>,
{
    ...
    fn common_point(&mut self, point: C) -> io::Result<()> {
        ...
        let coords: Coordinates<C> = Option::from(point.coordinates())
            .expect("cannot write points at infinity to the transcript");
...
impl<C: CurveAffine, T> gstd::ops::Deref for ChallengeScalar<C, T> {
...
impl<C: CurveAffine> gstd::ops::Deref for Challenge255<C> {
    type Target = [u8; 32];
