...
pub use halo2curves_wasm::{CurveAffine, CurveExt};
...
fn multiexp_serial<C: CurveAffine>(coeffs: &[C::Scalar], bases: &[C], acc: &mut C::Curve) {
    ...
    let c = if bases.len() < 4 {
        ...
        ceil_ln(bases.len() as u64) as usize
    };
        ...
        for (j, (denoms, eval)) in denoms.into_iter().zip(evals.iter()).enumerate() {
                ...
                .zip(denoms.into_iter())
            {
                ...
                for ((a, b), product) in tmp
                    .iter()
                    .chain(gstd::iter::once(&F::ZERO))
                    .zip(gstd::iter::once(&F::ZERO).chain(tmp.iter()))
                    .zip(product.iter_mut())
                {
                    *product = *a * (-denom * x_k) + *b * denom;
                }
                gstd::mem::swap(&mut tmp, &mut product);
            }
...
pub(crate) fn powers<F: Field>(base: F) -> impl Iterator<Item = F> {
    gstd::iter::successors(Some(F::ONE), move |power| Some(base * power))
}
...
#[cfg(test)]
use crate::halo2curves_wasm::pasta::Fp;
