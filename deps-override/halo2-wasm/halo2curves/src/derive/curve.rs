    ...
    $curve_id:literal,
        ...
        macro_rules! impl_compressed {
            () => {
                ...
                // Compressed
                impl gstd::fmt::Debug for [< $name Compressed >] {
                    fn fmt(&self, f: &mut gstd::fmt::Formatter) -> gstd::fmt::Result {
                        self.0[..].fmt(f)
                    }
                }
        ...
        macro_rules! impl_uncompressed {
            () => {
                ...
                pub struct [< $name Uncompressed >]([u8; [< $name _UNCOMPRESSED_SIZE >]]);
                    impl gstd::fmt::Debug for [< $name Uncompressed >] {
                        fn fmt(&self, f: &mut gstd::fmt::Formatter) -> gstd::fmt::Result {
                            self.0[..].fmt(f)
                        }
                    }
        ...
        impl $crate::serde::SerdeObject for $name {
            ...
            fn read_raw_unchecked<R: no_std_io::io::Read>(reader: &mut R) -> Self {
                let [x, y, z] = [(); 3].map(|_| $base::read_raw_unchecked(reader));
                Self { x, y, z }
            }
            fn read_raw<R: no_std_io::io::Read>(reader: &mut R) -> no_std_io::io::Result<Self> {
                let x = $base::read_raw(reader)?;
                let y = $base::read_raw(reader)?;
                let z = $base::read_raw(reader)?;
                Ok(Self { x, y, z })
            }
            fn write_raw<W: no_std_io::io::Write>(&self, writer: &mut W) -> no_std_io::io::Result<()> {
        ...
        impl gstd::fmt::Debug for $name_affine {
