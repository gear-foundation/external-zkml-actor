...
use external_actor_queue::{io::Event as ExtActorEvent, io::KzgParamsNoVec};
...
async fn client(program_id: ProgramId, api: GearApi, input_file_path: &str) {
    ...
    let payload = external_actor_queue::io::Incoming::Client(
        external_actor_queue::io::ClientMessage::CloneVkeyData,
    );
    let _ = send_message_and_wait_for_success(&api, &mut listener, program_id, payload).await;
...
async fn clean_verify(program_id: ProgramId, api: GearApi) {
    ...
    let payload = external_actor_queue::io::Incoming::Client(
        external_actor_queue::io::ClientMessage::PurgeVerification,
    );
    let _ = send_message_and_wait_for_success(&api, &mut listener, program_id, payload).await;
